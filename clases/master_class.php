...
class Master extends Miscelaneus
{
    ...
    public function getByNext($nombreProcedimiento, $parametros)
    {
        try {
            ...
            echo $e->getMessage();
            $this->setLog($e->getMessage(), 'Ocurrio en el: '.$nombreProcedimiento);
        } finally {
    ...
    public function insert($tabla, $attributes, $values, $intergers, $strings, $doubles, $nulls = array())
    {
        try {
    ...
    function delete($tabla, $attributes, $id_input)
    {
        try {
            ...
            if (count($error_tipo_dato) > 0) {
                $posiciones = implode(",", $error_tipo_dato);
                return "Error en tipo de datos. Posiciones ($posiciones)";
            }
