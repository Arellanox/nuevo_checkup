...
if (isset($datos['dispositivo']) && $datos['datos']) {
    ...
    $response = $master->insertByProcedure('sp_pseudo_interface', [json_encode($resultadosFinales)]);

    echo $master->returnApi(json_decode($response));


    // $fp = fopen('algo.txt', 'a');
    // fwrite($fp, json_encode($gh));
    // fclose($fp);
    ...
    // $fp = fopen('algo.txt', 'a');
    // fwrite($fp, json_encode($combinedArray));
    // fclose($fp);
...
function agruparPorPrefolio($datos)
{

    $grupos = array();

    foreach ($datos as $elemento) {
        $prefolio = $elemento['PREFOLIO'];
        if (array_key_exists($prefolio, $grupos)) {
            $grupos[$prefolio][] = $elemento;
        } else {
            $grupos[$prefolio] = array($elemento);
        }
    }
